...
export default function Navbar() {
    ...
    return (
        <header className="fixed top-0 w-full z-[1000] group/header">
            {/* --- BOLIVIAN TRICOLOR ACCENT --- */}
            ...
            <nav
                className={`w-full transition-all duration-700 ${isScrolled
                    ? 'py-3 md:py-4 bg-white/95 dark:bg-primary-950/95 backdrop-blur-md border-b border-slate-200 dark:border-white/5 shadow-sm'
                    : 'py-4 md:py-8 bg-white/10 dark:bg-black/10 backdrop-blur-sm'
                    }`}
                suppressHydrationWarning
            >
                <div className="max-w-[1700px] mx-auto px-6 md:px-20 flex items-center justify-between" suppressHydrationWarning>

                    {/* --- MAJESTIC BRANDING HIERARCHY --- */}
                    <Link href="/" className="flex items-center group gap-3 md:gap-5">

                        {/* Primary Identity (Ministry) - MONUMENTAL & STABLE */}
                        <div className="relative h-9 md:h-14 lg:h-16 transition-all duration-700 group-hover:scale-[1.02]" suppressHydrationWarning>
                            <img
                        ...
                        {/* Subtle Divider */}
                        <div className="h-6 md:h-10 w-px bg-slate-200 dark:bg-white/10" suppressHydrationWarning />

                        {/* Secondary Identity (Program) - RECTANGULAR & ELEGANT */}
                        <div className="relative h-5 md:h-8 group-hover:scale-105 transition-all duration-700" suppressHydrationWarning>
                            {institution.imagen ? (
                                <img
