        ...
        >
            <div className="space-y-40">
                    ...
                    {isLoading ? (
                        ...
                        sedes.map((sede, idx) => (
                            <motion.div
                                ...
                                className="group relative p-12 rounded-[4rem] bg-white dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 overflow-hidden transition-all duration-700 hover:shadow-2xl hover:shadow-primary-500/10"
                            >
                                ...
                                <div className="space-y-8 relative z-10">
                                    <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-primary-500 group-hover:bg-primary-500 group-hover:text-white transition-all duration-500">
                                        <MapPin className="w-7 h-7" />
                                    </div>
                                    <div>
                                        <h3 className="text-4xl font-black text-slate-950 dark:text-white tracking-tighter uppercase">{sede.nombre}</h3>
                                        <p className="text-[11px] font-black text-primary-500 tracking-[0.5em] mt-2">JURISDICCIÓN S{sede.abreviacion}</p>
                                    </div>

                                    <Link
                                        href={`/?tenant=${sede.abreviacion}`}
                                        className="inline-flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-hover:text-primary-500 transition-colors"
                                    >
                ...
                <section className="relative p-20 md:p-32 rounded-[6rem] bg-primary-950 text-white overflow-hidden text-center space-y-12">
                    <div className="absolute inset-0 bg-primary-500/10 blur-[150px] animate-pulse" />
                    <Globe className="w-32 h-32 mx-auto text-primary-500 opacity-50 animate-spin-slow" />
                    <div className="space-y-6 relative z-10">
                        <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter">Alcance <span className="text-primary-500">Plurinacional.</span></h2>
