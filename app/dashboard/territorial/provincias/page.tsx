...
export default function ProvinciasPage() {
    ...
    const [editingProvincia, setEditingProvincia] = useState<any>(null);
    ...
    const loadData = async () => {
        try {
            ...
            toast.error('Fallo en la sincronización de provincias');
        } finally {
            setLoading(false);
        }
    };

    const handleOpenModal = (prov: any = null) => {
        if (prov) {
            setEditingProvincia(prov);
            setFormData({
                nombre: prov.nombre,
            });
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingProvincia) {
                await provinciaService.update(editingProvincia.id, formData);
                toast.success('Provincia actualizada correctamente');
            } else {
                await provinciaService.create(formData);
                toast.success('Nueva provincia registrada');
            }
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            toast.success('Provincia removida');
            setIsDeleting(null);
            loadData();
        } catch (error) {
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-3">
                        ...
                        <span>Demarcación Geográfica</span>
                    </div>
                    <h1 className="text-4xl font-black tracking-tighter text-foreground uppercase leading-none">Provincias</h1>
                    <p className="text-sm font-medium text-muted-foreground max-w-lg">
                        Administración de las provincias registradas en el mapa nacional.
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <Can action="create" subject="Provincia">
                        <button
                                    ...
                                    <div className="p-6 space-y-6">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                                                <Can action="update" subject="Provincia">
                                                    <button
                                                ...
                                                <Can action="delete" subject="Provincia">
                                                    <button
                                                        onClick={() => setIsDeleting(prov.id)}
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Estado Activo</span>
                                            </div>
                                        </div>

                                        <div className="pt-4 border-t border-border/50 flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <Navigation className="w-3.5 h-3.5 text-muted-foreground/60" />
                                                <span className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">ID Técnico</span>
                                            </div>
            ...
            <Modal
                ...
                title={editingProvincia ? 'Actualizar Provincia' : 'Nueva Provincia'}
            >
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Nombre de la Provincia</label>
                            <input
                                type="text"
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-border">
                        ...
                        >
                            {editingProvincia ? 'Salvar Cambios' : 'Registrar'}
                        </button>
                    </div>
                </form>
            </Modal>
                ...
                <div className="space-y-8 text-center py-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-[13px] text-muted-foreground px-10 leading-relaxed font-bold italic opacity-70">
                            "Esta acción es irreversible y afectará los registros geográficos."
                        </p>
                    </div>
